...
BIN=bin
INCLUDE=
LIBS= -lcurl -lboost_regex -lasound -lopenal -lFLAC -ljsoncpp
TMP=tmp
INC= -Isource/ -Itarget/include/
LDPATH= -Ltarget/lib/

#list of files containing main() function, to prevent conflicts while compiling
MAINFILES:=source/console_command.cpp samples/sample-record.cpp source/voiceCommand-old.cpp samples/curlpost.cpp
...
.PHONY: sample
sample: sample-record voiceCommand-old curlpost
